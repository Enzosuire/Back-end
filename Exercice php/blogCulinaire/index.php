...
<body>
    ...
    <section class="p-5 container">
        ...
        <div class="row">
            ...
            $req = $db->query('SELECT * FROM t_recipe ORDER BY RAND() LIMIT 1');
            $recipe = $req->fetch()
            ?>
    ...
    <section class="p-5 container">
        ...
        <div class="row">
            <?php
            $req = $db->query('SELECT * FROM t_recipe ORDER BY rec_id DESC LIMIT 3');

            while ($recipes = $req->fetch()) { ?>
                <div class="col-sm-12 col-md-4">
                    <div class="card" style="width: 18rem;">
                        <img src="images/<?= $recipes['rec_miniature'];  ?>" class="card-img-top" alt="...">
                        <div class="card-body">
                            <h5 class="card-title"><?= $recipes['rec_nom']; ?></h5>
                            <p class="card-text"><?= $recipes['rec_resume']; ?></p>
                            <a href="single.php?id=<?= $recipes['rec_id']; ?>" class="btn btn-warning">Voir la recette</a>
                        </div>
                    </div>
                </div>
            <?php } ?>
        ...
        <div class="row">
            <div id="carouselExampleSlidesOnly col-sm-12 col-md-6 bg-carousel" class="carousel slide my-5" data-bs-ride="carousel">
                <div class="carousel-inner">
                    <?php
                    $req = $db->query('SELECT * FROM t_comment AS C INNER JOIN t_recipe AS R ON C.id_rec = R.rec_id ORDER BY com_id ASC LIMIT 3');
                    $db = null;
                            ...
                            <?php } else { ?>
                                ...
                                <div class="row d-flex justify-content-center">
                                    <div class="col-lg-8 text-center">
                                        <p class="fs-5"><?= $tuple['rec_nom'] ?></p>
                                        <p>
                                            <img class="rounded-circle shadow-1-strong mb-4" src="images/<?= $tuple['rec_miniature'] ?>" alt="avatar" style="width: 150px;" />
                                        </p>
                                        <p class="text-muted fs-2">
                                            <i class="bi bi-quote"></i>
                                            <?= $tuple['com_contenu'] ?>
                                        </p>
                                        <h2 class="mb-3 fs-3"><?= $tuple['com_auteur'] ?></h2>
                                    </div>
                                </div>
