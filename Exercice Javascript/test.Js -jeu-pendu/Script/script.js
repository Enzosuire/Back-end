   ...
   bouton.addEventListener("click", (detailEvenement) => {
      ...
      detailEvenement.target.disabled = true;
         ...
         if (!motTrouve) {
           ...
           essaisRestantsElement.textContent= essaisRestants+"essaisRestants";
           
        
     
           if (essaisRestants === 0) {
             console.log("Perdu !");
             document.getElementById("message").textContent = "Désolé, vous avez perdu ! Le mot mystère était '" + "'.";
           }
         }
     });
   })
